...
avg_nt_range = np.arange(first_avg_nt, last_avg_nt + avg_nt_stepsize, avg_nt_stepsize)
...
for nt_ind, avg_nt in enumerate(avg_nt_range):
    ...
    G_array[nt_ind, 1:] = [avg_G_A, avg_G_B, avg_G_C]

G_df = pd.DataFrame(data=G_array, columns=['lambda', 'A', 'B', 'C'])
G_df_tidy = pd.melt(G_df, ['lambda'], var_name='species', value_name='G')
ax = sns.lineplot(x="lambda", y="G", hue="species", data=G_df_tidy)
if SAVE_LAST_FIGURE:
