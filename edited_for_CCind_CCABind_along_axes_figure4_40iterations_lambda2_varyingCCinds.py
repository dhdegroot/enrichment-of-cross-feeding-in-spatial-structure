...
if not LOAD_DATA:
    ...
    CCAB_ind_factors = np.linspace(start_CCC_ind_factor, end_CCC_ind_factor, N_CCC_ind)
    ...
    G_df = pd.DataFrame(
        columns=['lambda', 'simulation', 'species', 'G', 'iteration', 'start_freq', 'CC0', 'CCC_ind', 'CCA_ind',
                 'CCB_ind',
                 'adv_cheat'])
    ...
    for simu_ind, CC_ind_factors in enumerate(simu_CC_inds):
        ...
        CCA_ind = CCB_ind = CCAB_ind_factor * CC0
        iter_dict_list = []
        for iter_ind in range(N_ITERATIONS):
            if iter_ind % 10 == 9:
                print("Computing '{0}'th iteration".format(iter_ind + 1))
            iter_dict = {'freqs': freqs_simu.copy(), 'CC0': CC0, 'CCC_ind': CCC_ind, 'CCA_ind': CCA_ind,
                         'CCB_ind': CCB_ind,
                         'adv_cheat': adv_cheat}
    ...
    G_df = pd.read_csv(
        os.path.join(working_dir, "results", "saved_results",
                     "figure4_dataG_df_40iterations_lambda2_varyingCCinds.csv"))
...
ax2 = sns.scatterplot(x='ratio_CCCind_CC0', y='ratio_CCABind_CC0', hue='start_freq',
                      data=G_df[(G_df.species == 'C') & (G_df.iteration == N_ITERATIONS - 1)], s=400)
...
plt.savefig(os.path.join(working_dir, "results", "figure4_Final cheater fraction after iterations.png"))
# plt.show()

# Make three figures for some specific choices of ratio_CCCind_CC0 and ratio_CCABind_CCCind
spec_coord = [[int(np.floor(0.5 * N_CCC_ind)), int(np.floor(0.2 * N_CCAB_ind))],
              [int(np.floor(0.2 * N_CCC_ind)), int(np.floor(0.8 * N_CCAB_ind))],
              [int(np.floor(0.5 * N_CCC_ind)), int(np.floor(0.8 * N_CCAB_ind))]]
first_df = G_df[(abs(G_df.ratio_CCABind_CCCind - np.unique(G_df.ratio_CCABind_CCCind)[spec_coord[0][1]]) <0.001) & (
        abs(G_df.ratio_CCCind_CC0 - np.unique(G_df.ratio_CCCind_CC0)[spec_coord[0][0]])<.001)].copy()
sec_df = G_df[(abs(G_df.ratio_CCABind_CCCind - np.unique(G_df.ratio_CCABind_CCCind)[spec_coord[1][1]]) < 0.001) & (
        abs(G_df.ratio_CCCind_CC0 - np.unique(G_df.ratio_CCCind_CC0)[spec_coord[1][0]]) < 0.001)].copy()
third_df = G_df[(abs(G_df.ratio_CCABind_CCCind - np.unique(G_df.ratio_CCABind_CCCind)[spec_coord[2][1]]) < 0.001) & (
            abs(G_df.ratio_CCCind_CC0 - np.unique(G_df.ratio_CCCind_CC0)[spec_coord[2][0]])<0.001)].copy()

selected_G_df = pd.concat([first_df, sec_df, third_df])

g = sns.FacetGrid(selected_G_df[selected_G_df.species != 'coop_advantage'], col="simulation", hue='species',
                  legend_out=True)
g = g.map(sns.lineplot, 'iteration', 'start_freq')
g.add_legend()
plt.savefig(os.path.join(working_dir, "results", "figure4_Time courses for selected parameters.png"))
# plt.show()

selected_G_df.to_csv(os.path.join(working_dir, "results", "figure4_selected_G_df_40iterations_lambda2.csv"),
                     index=False, header=True)
